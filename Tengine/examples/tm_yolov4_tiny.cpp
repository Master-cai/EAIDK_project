...
//    cv::destroyWindow("mywindow");
    ...
    /* check files */
    image_file="test.jpg";
    model_file ="yolov3_tiny_dk.tmfile";
    if (nullptr == model_file)
    {
        fprintf(stderr, "Error: fffffTengine model file not specified!\n");
        show_usage();
        return -1;
    }
...
//    cv::namedWindow("mywindow", CV_WINDOW_AUTOSIZE);
    ...
    readFile.open("./ip_port.txt", ios::in);
    std::string ip, port;
    readFile >> ip >> port;
    readFile.close();
  ...
  serv_addr.sin_port = htons(atoi(port.c_str()));                   //端口
  connect(sock, (sockaddr *)&serv_addr, sizeof(serv_addr));
   ...
   while(1)
    ...
    int i, j;
    json json_data;
    std::vector<std::string> made_json_vector;
    int box_num = 0;
    for (i = 0; i < nboxes; ++i)
    {
        int cls = -1;
        ...
        /*
        "[{\"left\":233,\"bottom\" : 73, \"right\" : 259, \"top\" : 185, \"classNo\" : 73, \"prob\" :  \"46%\" }, { \"left\":133,\"bottom\" : 13, \"right\" : 159, \"top\" : 85, \"classNo\" : 13, \"prob\" :  \"86%\"  }]";
        */
        if (cls >= 0)
        {
            box b = dets[i].bbox;
            ...
            //构造json数据
            std::string box_name = "box" + std::to_string(box_num + 1);
            json_data["boxes"][box_name]["left"] = left;
            json_data["boxes"][box_name]["right"] = right;
            json_data["boxes"][box_name]["top"] = top;
            json_data["boxes"][box_name]["bot"] = bot;
            json_data["boxes"][box_name]["class"] = cls;
            json_data["boxes"][box_name]["probability"] = dets[i].prob[j] * 100;

            //构造json_str数据
            std::string j_s = "{\"left\":" + std::to_string(left) +
             ",\"bottom\" : " + to_string(bot) +
             ", \"right\" : " + to_string(right) +
             ", \"top\" : " + std::to_string(top) +
             ", \"classNo\" : " + "\""+std::to_string(cls) +"\""+
             ", \"prob\" :  \"" + std::to_string(int(dets[i].prob[j] * 100)) + "%\" }";
            made_json_vector.push_back(j_s);
            box_num += 1;
    ...
    json_data["num"] = box_num;
    free(dets);
    save_image(img, "tengine_example_out");
    cv::Mat img1 = cv::imread("tengine_example_out.jpg", CV_LOAD_IMAGE_UNCHANGED);
    //socket 发送
    ...
    //    int send_image_num = send(sock, image_buff_send, image_size, 0);
    //int send_image_num = SendAll(sock, image_buff_send, image_size);
    //printf("image data sent %d bytes\n", send_image_num);

    std::string made_json_str;
    //构造json发送数据
    if(made_json_vector.size()==1){
        made_json_str = "["+made_json_vector[0]+"]";
    } else if(made_json_vector.size()>1){
        made_json_str = "[";
        for(int i = 0; i<made_json_vector.size()-1; i++){
            made_json_str += made_json_vector[i];
            made_json_str += ", ";
        }
        made_json_str += made_json_vector[made_json_vector.size()-1];
        made_json_str += "]";
    }
    //发送json
    ...
    // sprintf(json_data_length, "%04d", json_sent.length());
    sprintf(json_data_length, "%04d", made_json_str.length());

    std::string json_data_num = std::to_string(json_sent.length());
    printf("json data length %s\n", json_data_length);
    send(sock, json_data_length, 4, 0);
    sleep(1);
    printf("sleeping\n");
    // int sent_json_num = send(sock, json_sent.c_str(), json_sent.length(), 0);
    int sent_json_num = send(sock, made_json_str.c_str(), made_json_str.length(), 0);

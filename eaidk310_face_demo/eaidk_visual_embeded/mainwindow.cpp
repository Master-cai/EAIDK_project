                       ...
                       ui(new Ui::MainWindow)
{
	...
	video_port.push_back(-1);
        ...
        serv_addr.sin_addr.s_addr = inet_addr("202.206.212.106"); //具体的IP地址
...
//                    cv::imshow("test", realtime_face); //test
                    //cv::imwrite("/home/openailab/Desktop/test.jpg",realtime_face);
...
void MainWindow::open_camera()
{
	...
	for (int i=0; i<=10; i++)
        ...
        //printf("%s","1--------it is success");
        //char* fileDir = "rtsp://admin:LUAKGM@192.168.123.233:554/h264/ch1/main/av_stream";
        //if(cam->videoCapL.open(fileDir))
        if(cam->videoCapL.open(0))
    	{
            //printf("%s","2--------it is success");
